...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim = -1) # (Batch, h, seq_lemn, seq_len)
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048) -> Transformer:
    ...
    tgt_embedding = InputEmbeddings(d_model, tgt_embedding)
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attentio_block = MultiHeadAttentionBlock(d_model, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        encoder_block = EncoderBlock(encoder_self_attentio_block, feed_forward_block, dropout)
